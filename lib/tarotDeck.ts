...
function minorSuit(suit: string, slug: string): TarotCardBase[] {
  return [
    ["ace","Ace"],["two","Two"],["three","Three"],["four","Four"],["five","Five"],
    ["six","Six"],["seven","Seven"],["eight","Eight"],["nine","Nine"],["ten","Ten"],
    ["page","Page"],["knight","Knight"],["queen","Queen"],["king","King"],
  ].map(([s,label]) => ({ id:`${s}-of-${slug}`, name:`${label} of ${suit}`,
    image:`/cards/${s}-of-${slug}.jpg`, arcana:"Minor" as const, suit }))
}

export const LIVE_TAROT_DECK: TarotCardBase[] = [
  ...major,
  ...minorSuit("Wands","wands"), ...minorSuit("Cups","cups"),
  ...minorSuit("Swords","swords"), ...minorSuit("Pentacles","pentacles"),
]
...
export function drawTarotCards(count: number): TarotCardDraw[] {
  ...
  return deck.slice(0,count).map(c => ({ ...c, orientation: Math.random()<0.5 ? "upright" : "reversed" }))
}
