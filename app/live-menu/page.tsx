...
const MENU_ITEMS = [
  ...
  { tokens: "40",  label: "3 Card Tarot Reading",      desc: "Past · Present · Future" },
  ...
  return (
    <>
      <style>{`
        ...
        .menu-row:hover { background: rgba(201,168,76,0.04); }
      `}</style>
      ...
      <div style={{
        ...
        {/* ambient stars */}
        {[
          [8,12],[19,34],[33,7],[47,22],[62,5],[76,18],[89,9],
          [4,48],[25,61],[54,44],[71,58],[93,42],[13,77],[82,91],
        ].map(([x, y], i) => (
          <div key={i} style={{
            position: "absolute",
            left: `${x}%`, top: `${y}%`,
            ...
            animation: `floatStar ${3 + (i * 0.4) % 2}s ease-in-out infinite`,
        ...
        <div style={{
          ...
          {MENU_ITEMS.map((item) => (
            <div key={item.tokens + item.label} className="menu-row">
              <div style={{
                fontFamily: "'Cinzel Decorative', serif",
        ...
        <div style={{
          ...
          No medical, legal, financial, pregnancy, death, or guaranteed love predictions.
        </div>
      </div>
    </>
  )
