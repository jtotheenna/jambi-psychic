...
export async function POST(req: Request) {
  ...
  if (message === "__OPENING__") {
    const resp = await anthropic.messages.create({
      ...
      max_tokens: 110,
      system: `You are Galileo — ancient oracle who reads the heart. No asterisks. No stage directions.${userName ? ` The person's name is ${userName}.` : ""}
Welcome them warmly. Something about love or the heart is present — let them feel you sense it. Then ask: what is the heart carrying right now? What situation, person, or feeling have they brought with them? 2 sentences maximum. End with a question.${languageInstruction(language as Language)}`,
...
No asterisks. No bullet points. No stage directions. 4–5 sentences per response.${closingNote}
