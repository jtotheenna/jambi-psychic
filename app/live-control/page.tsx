...
export default function LiveControlPage() {
  ...
  const prompt = hasCards
    ? isTarot
      ? `Viewer question:\n${state.question.trim() || "(no question asked)"}\n\nTarot spread:\n${state.cards.map((c, i) =>
          `${activePositions[i]}: ${c.name} — ${c.orientation.charAt(0).toUpperCase() + c.orientation.slice(1)}`
      ...
      : `Viewer question:\n${state.question.trim() || "(no question asked)"}\n\nCartomancy spread:\n${state.cartoCards.map((c, i) =>
          `${activePositions[i]}: ${c.name} — ${c.orientation.charAt(0).toUpperCase() + c.orientation.slice(1)}\n  Meaning: ${c.meaning}`
        ).join("\n")}\n\nGive a mystical, emotionally intelligent cartomancy reading based on these playing cards. Cartomancy reads the cards as symbols and messengers — speak to the energy, the story, the pattern. Do not claim certainty. Keep it reflective and symbolic. Do not give medical, legal, financial, pregnancy, death, or guaranteed love predictions.`
    : ""
  ...
  const copyToSend = () => {
    ...
    const spreadLine = activeCards.map((c, i) =>
      `${activePositions[i]}: ${c.name} (${c.orientation})`
    ).join(" · ")
  ...
  const tarotBtn: React.CSSProperties = {
    ...btn, borderColor: "rgba(124,58,237,0.5)", background: "rgba(124,58,237,0.1)", color: "#a78bfa",
  }
  const cartoBtn: React.CSSProperties = {
    ...btn, borderColor: "rgba(220,64,64,0.5)", background: "rgba(220,64,64,0.08)", color: "#f87171",
  }
  ...
  return (
    <>
      ...
      <div style={{ minHeight: "100vh", padding: "20px 28px", display: "flex", flexDirection: "column", gap: 18, maxWidth: 860, margin: "0 auto" }}>
        ...
        {/* tarot draw */}
        <div className="section">
          <label>Tarot — Fisher-Yates shuffled, 78 cards</label>
          <div className="row">
        ...
        {/* cartomancy draw */}
        <div className="section">
          <label>Cartomancy — playing cards, 52 cards</label>
          <div className="row">
